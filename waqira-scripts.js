...
const supabase = Supabase.createClient(
  ...
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp3Z3Rja3BpdGd4ZW90eWx5eHRnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA0NDMzNDcsImV4cCI6MjA4NjAxOTM0N30.U3jZKNjzgqfG7BpTNz4NDp7iW59bmByhkej4o6835OU'
);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Interactive Image Tilt (unchanged, applied to non-product images)
  document.querySelectorAll('.home-images img, .gallery-item img:not([data-product-id])').forEach(el => {
    ...
    el.addEventListener('mousemove', e => {
      ...
      // Only apply transform if image is not inside a product gallery item with group-hover
    ...
    el.addEventListener('mouseleave', () => {
      // Only reset transform if image is not inside a product gallery item with group-hover
      if (!el.closest('.gallery-item')) {
  ...
  const navUl = document.getElementById('main-nav-list'); // Changed to use ID
  let menuOverlay = null; // Declare here to be accessible

  const toggleMobileMenu = (open) => {
    if (!navUl || !toggleBtn) return;

    const isOpen = typeof open === 'boolean' ? open : !navUl.classList.contains('active');
    ...
    if (isOpen) {
      ...
      if (!menuOverlay) {
        ...
        menuOverlay.addEventListener('click', () => toggleMobileMenu(false)); // Close menu
      }
      // RequestAnimationFrame for next frame to ensure transition
      ...
      if (menuOverlay) {
        ...
        }, 300); // Match CSS transition duration (opacity/visibility)
      }
  ...
  if (toggleBtn && navUl) {
    ...
    navUl.querySelectorAll('a').forEach(a => {
      a.addEventListener('click', () => {
        setTimeout(() => { // Small delay to allow navigation
          toggleMobileMenu(false); // Close the menu explicitly
        }, 150);
      });
    });
    ...
    window.addEventListener('resize', () => {
      if (window.innerWidth >= 768 && navUl.classList.contains('active')) {
        toggleMobileMenu(false); // Close the menu
      }
    });
  }
  ...
  // =============================================
  // CART LOGIC – FIXED & WORKING VERSION
  // =============================================
  ...
  const addToCart = (product) => {
    ...
    updateOrderPageCartTotal(); // Update order page total if on that page
  };
  ...
  document.addEventListener('click', (e) => {
    ...
    // Handle checkout button click for carty.html
  ...
  const updateOrderPageCartTotal = () => {
    const orderPageTotalEl = document.getElementById('order-page-cart-total');
    const payButtonTotalEl = document.getElementById('pay-button-total');
    const total = getCartTotal().toFixed(2);
    if (orderPageTotalEl) orderPageTotalEl.textContent = total;
    if (payButtonTotalEl) payButtonTotalEl.textContent = total;
  };
  ...
  if (orderForm) {
    ...
    // Initial update of total on order page load
    updateOrderPageCartTotal();
  }
  ...
  if (document.body.classList.contains('admin-page')) {
    ...
    const AUTH_KEY = 'waqira_admin_logged_in';
  ...
  // Product CRUD (placeholder, functions need to be defined globally or in scope here if used)
